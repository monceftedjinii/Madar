  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setErrorMessage("Impossible de charger le dashboard RH.");
      }
    };
      ...
      <div className="profile-content !h-auto min-h-screen bg-transparent">
        ...
        <main className="mx-auto flex w-[96%] flex-col gap-6 py-6">
          <DashboardHeader
            ...
            description="Centralisez les validations, les effectifs, les documents, les formations et les evaluations depuis un seul espace RH."
          ...
          <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
            <article className={cardClass}>
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                {dashboardData.profile.avatar ? (
                  ...
                  <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-2xl font-extrabold uppercase tracking-[0.12em] text-white shadow-md">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Employes</p>
                  <p className={`mt-3 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
                    {dashboardData.profile.employeesCount ?? 0}
                  </p>
                </div>
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">En ligne</p>
                  <p className={`mt-3 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
                    {dashboardData.profile.onlineCount ?? 0}
                  </p>
                </div>
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Conges RH</p>
                  <p className={`mt-3 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
                    {dashboardData.profile.pendingLeaves ?? 0}
                  </p>
                </div>
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Docs a valider</p>
                  <p className={`mt-3 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
            ...
            <article className={cardClass}>
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className={`mt-1 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                    Priorites visibles sur la periode selectionnee.
                  </p>
                </div>
              ...
              <div className="space-y-4">
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Conges a traiter</p>
                  <p className={`mt-2 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
                    {dashboardData.charts.leaves.pending}
                  </p>
                </div>
                <div className={innerClass}>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Demandes formation</p>
                  <p className={`mt-2 text-2xl font-bold ${dark ? "text-slate-50" : "text-slate-900"}`}>
            ...
            <article className={cardClass}>
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className={`mt-1 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                    Effectif visible service par service.
                  </p>
                </div>
            ...
            <article className={cardClass}>
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className={`mt-1 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                    Demandes traitees sur la periode.
                  </p>
                </div>
          ...
          <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <article className={cardClass}>
              <div className="mb-5">
                ...
                <p className={`mt-1 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                  Dernieres fiches employees visibles dans votre scope RH.
                </p>
              </div>
                  ...
                  <tbody>
                    {dashboardData.employees.map((employee) => (
                      <tr key={employee.id} className={dark ? "rounded-2xl bg-slate-800" : "rounded-2xl bg-slate-50"}>
                        ...
                        <td className={`px-3 py-4 text-sm ${dark ? "text-slate-300" : "text-slate-600"}`}>{employee.service}</td>
                        <td className={`px-3 py-4 text-sm ${dark ? "text-slate-300" : "text-slate-600"}`}>{employee.position}</td>
                        <td className="rounded-r-2xl px-3 py-4 text-sm">
                          <span className={`badge ${employee.isOnline ? "badge-termine" : "badge-attente"}`}>
                    ...
                    {!dashboardData.employees.length && (
                      <tr>
                        <td colSpan="4" className={`px-3 py-4 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                          Aucun employe recent a afficher.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <article className={cardClass}>
              <div className="mb-5">
                ...
                <p className={`mt-1 text-sm ${dark ? "text-slate-300" : "text-slate-500"}`}>
                  Activite recente remontee au profil RH.
                </p>
              </div>
